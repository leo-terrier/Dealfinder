...
export const pool = devEnv ? new Pool({
  user: 'leoterrier',
  host: 'localhost',
  database: 'dealFinder',
...
export const authUser =  (username, password, done) => {
  pool.query('SELECT * FROM users WHERE email = $1', [username], async (err, results) => { 
...
export const createUser = async (req, res, next)=> {
  ...
  const { firstName, lastName, username, password } = req.body
    ...
    pool.query('INSERT INTO users (first_name, last_name, email, password) VALUES ($1, $2, $3, $4) RETURNING *' , [firstName, lastName, username, hash], (error, results) => {
      try {
        if (error) {
          throw error
        }
...
export const addSavedSearch = (req, res, next) => {
  const {search_id, 
    ...
    pool.query('INSERT INTO savedSearches VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *', 
...
export const addSavedDeal = (req, res) => {
  const {deal_id,
    ...
    pool.query('INSERT INTO savedDeals VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *', 
...
export const getSearches = (req, res) => {
  pool.query('SELECT * FROM savedSearches WHERE user_id = $1', [req.user.user_id], (error, results) => {
    if (error) {
      throw error
    }
    console.log('Getting saved searches from user with id : ' + req.user.user_id );
...
export const getDeals = (req, res) => {
  ...
  pool.query('SELECT * FROM savedDeals WHERE user_id = $1', [req.user.user_id], (error, results) => {
    if (error) {
      throw error
    }
    console.log('Getting saved deals from user with id : ' + req.user.user_id );
...
export const deleteSavedSearch = (req, res) => {
  pool.query('DELETE FROM savedSearches WHERE search_id= $1', [req.params.search_id]), (error, results) => {
    if(error){
      throw error
    };
  };
...
export const deleteSavedDeal = (req, res) => {
  pool.query('DELETE FROM savedDeals WHERE deal_id= $1 AND user_id= $2', [req.params.deal_id, req.user.user_id]), (error, results) => {
    if(error){
      throw error
    };
  };
