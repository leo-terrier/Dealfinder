...
export function capitalizeFirstLetter(string) {
  return string.toLowerCase().split(' ').map(str => str[0].toUpperCase()+ str.slice(1)).join(' ')
}

export function neighborDealsCoordinates (deals) {

//get the set of coordinates per deal, inside an array
  const coordinates = deals.map(deal => [deal.geo_point.lon, deal.geo_point.lat])
...
//Get all deals which have neighbors in their coordinates in an array of object
  ...
  deals.forEach(deal => {
    ...
    const firstIndex = coordinates.findIndex(c => c[0] === arr[0] && c[1] ===arr[1]);
    const lastIndex = coordinates.reverse().findIndex(c => c[0] === arr[0] && c[1] ===arr[1]) ; 
