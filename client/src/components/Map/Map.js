...
const { boxContainer, mapCard, OutterInputBox, fieldBox, input, filterButton, innerInputBox, fabFilter, refreshBox, refreshButton, fiberIcon, tooltip } = style;
...
const Marker = ({ deal, dealOnMap, setDealOnMap, neighborDealsPerCoordinates, addDeal, savedDeals, removeDeal, key, setMarkersMounting, markersMounting, isLoadingMap, dragCount }) => {
  ...
  const title = <TooltipContent neighbors={neighbors.reverse()} deal={deal} setDealOnMap={setDealOnMap} addDeal={addDeal} savedDeals={savedDeals} removeDeal={removeDeal} />;

  useEffect(() => {
    !isLoadingMap && setMarkersMounting(false);
  }, [markersMounting, dragCount]);

  return (
    <Tooltip title={title} open={isDealOnMap} arrow sx={tooltip} key={key}>
      <FiberManualRecordIcon
        key={key}
        sx={{ ...fiberIcon, display: markersMounting && "none" }}
        color={neighbors.findIndex((neighbor) => neighbor === dealOnMap) !== -1 ? "secondary" : "primary"}
        fontSize="small"
        onClick={(e) => {
          e.stopPropagation();
...
const Map = ({ coordinates, deals, setDealOnMap, dealOnMap, addDeal, savedDeals, removeDeal, getResultsFromCoordinates, minSurface, maxSurface, minPrice, maxPrice }) => {
  ...
  return (
    <Box sx={boxContainer}>
      <Fab
        ...
        sx={fabFilter}
        onClick={() => {
          setIsFiltersOpen((prev) => (prev ? false : true));
        }}
      >
        {" "}
        {isFiltersOpen ? <FilterAltOffOutlinedIcon sx={filterButton} /> : <TuneOutlinedIcon sx={filterButton} />}
      </Fab>
      <Box sx={{ ...OutterInputBox, display: isFiltersOpen ? "flex" : "none" }}>
        <Typography variant="h4" sx={{ fontFamily: "Rampart One", color: "primary.main" }}>
          Filters
        </Typography>
        <Box sx={innerInputBox} component="form">
          <Box sx={fieldBox}>
            ...
            <input
              ...
              value={minPriceMap}
              onChange={(e) => {
                setMinPriceMap(e.target.value);
              }}
            />
          </Box>
          <Box sx={fieldBox}>
            <label htmlFor="maxPrice">Max Price :</label>
            <input style={input} name="maxPrice" id="maxPrice" type="number" variant="outlined" value={maxPriceMap} onChange={(e) => setMaxPriceMap(e.target.value)} />
          </Box>
          <Box sx={fieldBox}>
            <label htmlFor="minSurface">Min Surface :</label>
            <input id="minSurface" style={input} name="minSurface" type="number" variant="outlined" value={minSurfaceMap} onChange={(e) => setMinSurfaceMap(e.target.value)} />
          </Box>
          <Box sx={fieldBox}>
            <label htmlFor="maxSurface">Max Surface :</label>
            <input style={input} name="maxSurface" id="maxSurface" type="number" variant="outlined" value={maxSurfaceMap} onChange={(e) => setMaxSurfaceMap(e.target.value)} />
          </Box>
        </Box>
        <Box sx={refreshBox}>
          <Button size="small" variant="outlined" sx={refreshButton} onClick={() => fetchResultsFromMap()}>
            Refresh
          </Button>
        </Box>
      </Box>
