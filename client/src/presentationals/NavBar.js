  ...
  const logout = async () => {
    ...
    const loginAndOut = () => {
      ...
      if (!greaterThanSm && !firstName){
        return  <IconButton component={Link} to="login" sx={iconButton}><LoginIcon /></IconButton>
      }
      if(!greaterThanSm && firstName){
        return <IconButton sx={iconButton}><LogoutIcon onClick={logout}/></IconButton>
      }
      if (firstName) {
        return <Typography sx={{}}>Welcome {firstName} (<a href="#" onClick={logout}>log out</a>) </Typography>
      }else{
        return ( <Box sx={boxRight}>
        <Button sx={button} to="register" component={Link}> Register</Button>
